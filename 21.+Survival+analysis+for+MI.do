...
gen outcome = min(admission_mi, dod, follow_up)
gen failure =0 if admission_mi ==. & dod==.
replace failure = 1 if failure ==.

gen outcome2=min(admission_mi, dod, follow_up)
gen failure2=1 if failure==1 & (admission_mi<follow_up | dod<follow_up)
recode failure2 .=0
...
keep if _merge ==3
drop _merge
save "P:\ORD_Sundaram_202108013D\Padmini\Diabetes Stata Files\GLP1 new files hfief\final_survival_analysis_data_mi_hospitalization.dta", replace


*** STEP 6: Survival analyses 
clear all
use "P:\ORD_Sundaram_202108013D\Padmini\Diabetes Stata Files\GLP1 new files hfief\final_survival_analysis_data_mi_hospitalization.dta",
stset outcome2, id(scrssn) origin(dof) failure(failure2==1) exit(failure ==1 time td(1Jan2023)) scale(365.25)
